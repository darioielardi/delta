...
// the pill is tapped from an ineligible state, a notice explains why. (#guide)
const GUIDE_SCOPE_TITLE = "Walkthrough reviews your branch vs base";
...
const GUIDE_TOO_SMALL_MSG = "There’s not enough here to walk through — a walkthrough needs at least a couple of changed files. Just read the diff directly.";
const GUIDE_NOT_INSTALLED_MSG = "The walkthrough is generated by the Claude Code CLI, which isn’t on your PATH. Install it, then try again.";
// Mirror the backend floor (walkthrough/context.rs): a walkthrough needs ≥2 files and
// ≥20 changed lines, else there's nothing to orient. (#guide)
const GUIDE_MIN_CHANGED_LINES = 20;
const GUIDE_MIN_FILES = 2;
...
export function Workspace({ target, onOpenPalette, onOpenSettings }: { target: Target; onOpenPalette?: () => void; onOpenSettings?: () => void }) {
  ...
  // Whether the local `claude` CLI is present — gates the walkthrough pill. null =
  // still checking; the pill shows unless claude is definitively absent. (#guide)
  ...
  // Set to a {title, message} when the pill is tapped from an ineligible state.
  const [guideBlockedMsg, setGuideBlockedMsg] = useState<{ title: string; message: string } | null>(null);
  ...
  // Probe for the `claude` CLI once, to gate the walkthrough pill. (#guide)
  useEffect(() => {
    let cancelled = false;
    void api.claudeStatus().then(
      (s) => { if (!cancelled) setClaudeInstalled(s.installed); },
  ...
  const startWalkthrough = useCallback(async () => {
    if (!viewTarget || !review) return;
    // Supersede any in-flight generation for this review (frees its claude child).
    void api.cancelWalkthrough(review.id).catch(() => {});
    setGuideMode(true);
    setWalkthrough(null);
    setGuiding(true);
    try {
      ...
      // A user-initiated cancel rejects with "Cancelled." — not an error to surface.
  ...
  // Min-diff floor (mirrors the backend): a walkthrough needs ≥2 files and ≥20 changed
  // lines, else there's nothing to orient. (#guide)
  const guideTooSmall =
    orderedFiles.length < GUIDE_MIN_FILES ||
    orderedFiles.reduce((n, f) => n + (f.binary ? 0 : f.additions + f.deletions), 0) < GUIDE_MIN_CHANGED_LINES;
  // First click shows a one-time confirm (it spends Claude credits); after that it
  // goes straight in. Each ineligible state explains itself in a notice. (#guide)
  const requestWalkthrough = useCallback(() => {
    if (claudeInstalled === false) { setGuideBlockedMsg({ title: "Claude Code CLI not found", message: GUIDE_NOT_INSTALLED_MSG }); return; }
    if (dirty) { setGuideBlockedMsg({ title: GUIDE_SCOPE_TITLE, message: GUIDE_DIRTY_MSG }); return; }
    if (inCommitMode || !(diffMode === "all-changes" || diffMode === "branch-vs-base")) { setGuideBlockedMsg({ title: GUIDE_SCOPE_TITLE, message: GUIDE_MODE_MSG }); return; }
    if (guideTooSmall) { setGuideBlockedMsg({ title: "Nothing to walk through", message: GUIDE_TOO_SMALL_MSG }); return; }
    if (localStorage.getItem("delta.guide.confirmed") === "1") void startWalkthrough();
    else setGuideConfirmOpen(true);
  }, [claudeInstalled, dirty, inCommitMode, diffMode, guideTooSmall, startWalkthrough]);
  ...
  const exitGuide = useCallback(() => {
    setGuideMode(false);
    // Stop an in-flight generation if the user bails mid-way (frees the claude child).
    if (review) void api.cancelWalkthrough(review.id).catch(() => {});
  }, [review]);
  ...
  // Best-effort: kill an in-flight generation if the window is closing. (#guide)
  useEffect(() => {
    if (!review) return;
    const reviewId = review.id;
    const onUnload = () => { void api.cancelWalkthrough(reviewId); };
  ...
  return (
    <div data-testid="app-root" className="flex h-screen flex-col overflow-hidden bg-background text-[13px] text-foreground">
              ...
              {guideMode ? (
                ...
                <>
                  <FilesPanel
                    ...
                    padBottom={claudeInstalled !== false}
                  />
      ...
      <NoticeDialog
        ...
        title={guideBlockedMsg?.title ?? ""}
        message={guideBlockedMsg?.message}
